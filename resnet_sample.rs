...
use candle_lora as lora:
use candle_lora::lora::LoraModule;
use tracing::Span;
...
pub struct ResnetBlock2D {
    norm1: lora::Lora<nn::GroupNorm>,
    conv1: lora::Lora<Conv2d>,
    norm2: lora::Lora<nn::GroupNorm>,
    conv2: lora::Lora<Conv2d>,
    time_emb_proj: Option<lora::Lora<nn::Linear>>,
    conv_shortcut: Option<lora::Lora<Conv2d>>,
    span: tracing::Span,
    config: ResnetBlock2DConfig
}

#[lora::auto_lora_convert] // Use the macro
impl ResnetBlock2D {
  pub fn new(
    vs: nn::VarBuilder,
    in_channels: usize,
    ...
    let norm1 = lora::Lora::from_original(nn::group_norm(config.groups, in_channels, config.eps, vs.pp("norm1"))?);
    let conv1 = lora::Lora::from_original(conv2d(in_channels, out_channels, 3, conv_cfg, vs.pp("conv1"))?);
    let groups_out = config.groups_out.unwrap_or(config.groups);
    let norm2 = lora::Lora::from_original(nn::group_norm(groups_out, out_channels, config.eps, vs.pp("norm2"))?);
    let conv2 = lora::Lora::from_original(conv2d(out_channels, out_channels, 3, conv_cfg, vs.pp("conv2"))?);
    ...
    let conv_shortcut = if use_in_shortcut {
      ...
      Some(lora::Lora::from_original(conv2d(
        in_channels,
        out_channels,
        1,
        conv_cfg,
    ...
    let time_emb_proj = match config.temb_channels {
      None => None,
      Some(temb_channels) => Some(lora::Lora::from_original(nn::linear(
        temb_channels,
        out_channels,
        vs.pp("time_emb_proj"),
      )?)),
