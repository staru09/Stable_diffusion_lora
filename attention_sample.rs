...
pub struct AttentionBlock {
    group_norm: lora::Lora<nn::GroupNorm>,
    query: lora::Lora<nn::Linear>,
    key: lora::Lora<nn::Linear>,
    value: lora::Lora<nn::Linear>,
    proj_attn: lora::Lora<nn::Linear>,
    channels: usize,
    num_heads: usize,
    span: tracing::Span,
...
impl AttentionBlock {
    pub fn new(vs: nn::VarBuilder, channels: usize, config: AttentionBlockConfig) -> Result<Self> {
      ...
      let group_norm = lora::Lora::from_original(nn::group_norm(config.num_groups, channels, config.eps, vs.pp("group_norm"))?);
      let (q_path, k_path, v_path, out_path) = if vs.contains_tensor("to_q.weight") {
        ("to_q", "to_k", "to_v", "to_out.0")
      } else {
        ("query", "key", "value", "proj_attn")
      };
      let query = lora::Lora::from_original(nn::linear(channels, channels, vs.pp(q_path))?);
      let key = lora::Lora::from_original(nn::linear(channels, channels, vs.pp(k_path))?);
      let value = lora::Lora::from_original(nn::linear(channels, channels, vs.pp(v_path))?);
      let proj_attn = lora::Lora::from_original(nn::linear(channels, channels, vs.pp(out_path))?);
